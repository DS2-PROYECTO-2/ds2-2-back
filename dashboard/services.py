...
class DashboardService:
    ...
    def _get_user_room_usage_data(user):
        ...
        try:
            ...
            # Filtrar solo registros completos (con salida registrada)
            entries_by_room = RoomEntry.objects.filter(
                user=user,
                entry_time__gte=thirty_days_ago,
                exit_time__isnull=False,  # Solo registros con salida
                exit_time__gt=F('entry_time')  # Validar que salida > entrada
            ).values('room__name').annotate(
                total_sessions=Count('id'),
                total_hours=Sum(F('exit_time') - F('entry_time'))
            )
